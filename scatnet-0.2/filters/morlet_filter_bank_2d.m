...
function filters = morlet_filter_bank_2d(size_in, options,isGPU,isComplex)
    if(nargin<2)
		options = struct;
    end
if(nargin<3)
...
%	a = 2^(1/Q);
%    sigma_phi=sqrt(-2*log(tau_as))/(xi_psi)*(a+1)/(a-1);
%    sigma_psi=sqrt(-2*log(tau_lc/2))/(xi_psi - sqrt(-2*log(tau_lc))/sigma_phi);
